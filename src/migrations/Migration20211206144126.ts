...
export class Migration20211206144126 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "sales" ("id" serial primary key, "created_at" timestamptz(0) not null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "quantity" int4 not null, "price" int4 not null, "discount" int4 null, "product_id" int4 not null, "vat" int4 null);');

    this.addSql('create table "product_detail" ("id" serial primary key, "created_at" timestamptz(0) not null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "name" varchar(50) not null, "price" int4 not null, "product_id" int4 null);');
    this.addSql('alter table "product_detail" add constraint "product_detail_product_id_unique" unique ("product_id");');
    ...
    this.addSql('create table "bill" ("id" serial primary key, "created_at" timestamptz(0) not null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "image" varchar(250) not null, "sale_id" int4 not null);');
    this.addSql('alter table "bill" add constraint "bill_sale_id_unique" unique ("sale_id");');

    this.addSql('alter table "sales" add constraint "sales_product_id_foreign" foreign key ("product_id") references "product" ("id") on update cascade on delete cascade;');

    this.addSql('alter table "product_detail" add constraint "product_detail_product_id_foreign" foreign key ("product_id") references "product" ("id") on update cascade on delete set null;');
    ...
    this.addSql('alter table "bill" add constraint "bill_sale_id_foreign" foreign key ("sale_id") references "sales" ("id") on update cascade;');
  }
